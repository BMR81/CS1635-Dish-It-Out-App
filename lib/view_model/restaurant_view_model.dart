...
class RestaurantViewModel {
  Future<List<restaurant_model>> fetchRestaurants() async {
    final response =
        await rootBundle.loadString('assets/datasets/restaurants.json');
    final json = jsonDecode(response) as List<dynamic>;

    final restaurantList =
        json.map((e) => restaurant_model.fromJson(e)).toList();
  ...
  Future<List<restaurant_model>> getMatches(user_model currUser) async {
    ...
    for (var restaurant in restaurants) {
      if (restaurant.dish.toString() == currUser.firstDish.toString() ||
